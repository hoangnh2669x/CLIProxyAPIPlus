...
// ClaudeCodeResponseRewriter wraps a gin.ResponseWriter to intercept and modify the response body
// It's used to rewrite model names in responses when model mapping is used
type ClaudeCodeResponseRewriter struct {
	gin.ResponseWriter
	body          *bytes.Buffer
...
// NewClaudeCodeResponseRewriter creates a new response rewriter for model name substitution
func NewClaudeCodeResponseRewriter(w gin.ResponseWriter, originalModel string) *ClaudeCodeResponseRewriter {
	return &ClaudeCodeResponseRewriter{
		ResponseWriter: w,
		body:           &bytes.Buffer{},
...
func (rw *ClaudeCodeResponseRewriter) enableStreaming(reason string) error {
	if rw.isStreaming {
		return nil
	}
	rw.isStreaming = true
	...
	log.Debugf("claudecode response rewriter: switched to streaming (%s)", reason)
	return nil
}

// Write intercepts response writes and buffers them for model name replacement
func (rw *ClaudeCodeResponseRewriter) Write(data []byte) (int, error) {
	...
	if !rw.isStreaming {
		...
		if looksLikeSSEChunk(data) {
			...
			log.Warnf("claudecode response rewriter: buffer exceeded %d bytes, switching to streaming", maxBufferedResponseBytes)
...
func (rw *ClaudeCodeResponseRewriter) Flush() {
	if rw.isStreaming {
		if flusher, ok := rw.ResponseWriter.(http.Flusher); ok {
			flusher.Flush()
		}
	...
	if rw.body != nil && rw.body.Len() > 0 {
		if _, err := rw.ResponseWriter.Write(rw.rewriteModelInResponse(rw.body.Bytes())); err != nil {
			log.Warnf("claudecode response rewriter: failed to write rewritten response: %v", err)
		}
	}
}

// modelFieldPaths lists all JSON paths where model name may appear
var claudeCodeModelFieldPaths = []string{"model", "modelVersion", "response.modelVersion", "message.model"}

// rewriteModelInResponse replaces all occurrences of the mapped model with the original model in JSON
func (rw *ClaudeCodeResponseRewriter) rewriteModelInResponse(data []byte) []byte {
	if rw.originalModel == "" {
		return data
	}
	for _, path := range claudeCodeModelFieldPaths {
		if gjson.GetBytes(data, path).Exists() {
			var err error
			data, err = sjson.SetBytes(data, path, rw.originalModel)
			if err != nil {
				log.Warnf("claudecode response rewriter: failed to rewrite %s: %v", path, err)
			}
		}
	}
	return data
...
func (rw *ClaudeCodeResponseRewriter) rewriteStreamChunk(chunk []byte) []byte {
	if rw.originalModel == "" {
		return chunk
	}
